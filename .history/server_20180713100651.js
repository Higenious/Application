...
app.post('/api/emp/update', function(req, res){
    var data  = req.body;
    try {
        empModel.findOneAndUpdate({ "_id": data.employeeId }, { $set: data }, { new: true, upsert: true }, function (error, result) {
            if (error) {
                errorData(error)
            } else {
    ...
    try {
        empModel.findById(id)
            .then(function (result) {
                if (result) {
                    try {
                        empModel.remove({ _id: id })
                            .then(function (result) {
                                res.send({status: 200, result, message : "User Successfully record"});
                            }).catch(function (error) {
                                res.send(error)
                            });
                    } catch (error) {
